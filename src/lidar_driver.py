...
class LidarDriver:
    ...
    def diag(self) -> dict:
        ...
        # Estructura normalizada de retorno
        return {
            'model': info.get('model', 'Unknown'),
            'firmware': info.get('firmware', (0, 0)),
            'hardware': info.get('hardware', 0),
            'status': health[0] if health else 'Unknown',
            'error_code': health[1] if health else -1,
            '_raw_info': info,
            '_raw_health': health,
        }
    ...
    def frames(self, max_buf_meas: int = 500) -> Iterable[ScanFrame]:
        ...
        for scan in self.lidar.iter_scans(max_buf_meas=max_buf_meas):
            pts: List[ScanPoint] = []
            for q, a, d in scan:
                # Filtros de distancia y calidad
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Aseguramos el apagado incluso si hay un error en el loop
        d.shutdown_safe()
